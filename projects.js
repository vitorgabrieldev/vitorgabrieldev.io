(function() {
  ...
  function buildFilters() {
    const cats = ['all', ...new Set(D.projects.map(p => p.category.en))];
    filterBar.innerHTML = cats.map(c => {
      const n = c === 'all' ? D.projects.length : D.projects.filter(p => p.category.en === c).length;
      const label = c === 'all'
        ? window.t('projects.all')
        : window.td(D.projects.find(p => p.category.en === c)?.category) || c;
      return `<button class="filter-btn ${c===filter?'active':''}" data-filter="${c}">${label}<span class="c">${n}</span></button>`;
    }).join('');
  }
  ...
  function filtered() {
    return filter === 'all' ? D.projects : D.projects.filter(p => p.category.en === filter);
  }

  function render() {
    const items = filtered();
    const n = items.length;
    countBadge.textContent = `${n} ${n === 1 ? window.t('projects.results.one') : window.t('projects.results.many')}`;
  ...
  function renderList(items) {
    root.innerHTML = `<div class="plist">${items.map((p, i) => `
      <a href="project.html?id=${p.id}" data-link>
        ...
        <span class="plist__name">${p.name}<span class="cat">${window.td(p.category)}</span></span>
        <span class="plist__desc">${window.td(p.summary)}</span>
    ...
    root.innerHTML = `<div class="pgrid">${items.map((p, i) => `
      <a href="project.html?id=${p.id}" data-link>
        ...
        <div class="pgrid__body">
          <div class="pgrid__meta"><span>${window.td(p.category)}</span><span>${p.year}</span></div>
          <div class="pgrid__name">${p.name}</div>
          <div class="pgrid__desc">${window.td(p.summary)}</div>
    ...
    root.innerHTML = `<div class="pgall">${items.map((p, i) => `
      <a href="project.html?id=${p.id}" data-link>
        ...
        <div>
          ...
          <div class="pgall__desc">${window.td(p.summary)}</div>
          <div class="pgall__stack">${p.stack.map(s => `<span>${s}</span>`).join('')}</div>
          <div class="pgall__link">${window.t('projects.read_case')}</div>
        </div>
      </a>
    `).join('')}</div>`;
  }
